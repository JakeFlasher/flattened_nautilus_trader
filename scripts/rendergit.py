...
or export directly to an LLM-friendly text format.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Flatten a GitHub repo to a single HTML page or LLM context file")
    ...
    ap.add_argument("--llm-out", help="Output file path for LLM-optimized text (CXML format). If set, HTML generation is skipped unless --out is also specified.")
    ...
    # Determine if we should generate HTML
    # Logic: If user specifically asks for LLM output, they probably don't want the temp HTML
    # unless they explicitly asked for that too.
    generate_html = True
    if args.llm_out and args.out is None:
        generate_html = False

    # Set default output path for HTML if we are generating it and no path was provided
    if generate_html and args.out is None:
    ...
    try:
        ...
        # 1. Handle LLM Output (Direct Text)
        if args.llm_out:
            print(f"🤖 Generating LLM context (CXML)...", file=sys.stderr)
            cxml_text = generate_cxml_text(infos, repo_dir)
            llm_path = pathlib.Path(args.llm_out)
            llm_path.write_text(cxml_text, encoding="utf-8")
            print(f"💾 Wrote LLM context to: {llm_path.resolve()} ({bytes_human(llm_path.stat().st_size)})", file=sys.stderr)

        # 2. Handle HTML Output
        if generate_html:
