...
# --- START SURGICAL PATCH ---
# Fixes AttributeError: 'str' object has no attribute '__name__' in customdataclass
import pyarrow as pa
from typing import get_type_hints

_original_customdataclass = customdataclass


def customdataclass(*args, **kwargs):
    def _inject_schema(cls):
        # Pre-calculate _schema to bypass the buggy logic in the original decorator
        if not hasattr(cls, "_schema"):
            # Map copied from nautilus_trader source
            type_mapping = {
            ...
            # Resolve string annotations to actual types (e.g. "str" -> <class 'str'>)
            try:
                # Use globals() to resolve custom types like InstrumentIdCls
                hints = get_type_hints(cls, globalns=globals())
            except Exception:
                hints = {}

            fields = {}
            # Iterate local annotations only, matching original behavior
            for attr in getattr(cls, "__annotations__", {}):
                # Use resolved type from hints if available, else raw annotation
                typ = hints.get(attr, cls.__annotations__[attr])
                # Safely get name (handles both types and strings)
                t_name = getattr(typ, "__name__", str(typ))

                if t_name in type_mapping:
                    fields[attr] = type_mapping[t_name]

            # If we found fields, generate the schema so the original code skips it
            if fields:
                fields.update(
                    {
                        "type": pa.string(),
                ...
                cls._schema = pa.schema(fields)

    # Handle both @customdataclass and @customdataclass(...) usage
    if args and isinstance(args[0], type):
        cls = args[0]
        _inject_schema(cls)
        return _original_customdataclass(cls)

    def wrapper(cls):
        _inject_schema(cls)
        return _original_customdataclass(*args, **kwargs)(cls)
...
def run_etl(cfg: EtlConfig) -> EtlOutputs:
    ...
    # Optional instrument specs for precision enforcement (Bars + ticks).
    specs = None
    ...
    def _coerce_price_obj(x: Any, prec: int) -> Any:
        N = load_nautilus_imports()
        Price = N.Price
        d = _quantize(price_to_decimal(x), int(prec))
        return Price.from_str(str(d))

    def _coerce_qty_obj(x: Any, prec: int) -> Any:
        N = load_nautilus_imports()
        Quantity = N.Quantity
        d = _quantize(qty_to_decimal(x), int(prec))
        return Quantity.from_str(str(d))

    def _coerce_quote_precision(tick: Any, spec: Any) -> Any:
        N = load_nautilus_imports()
        QuoteTick = N.QuoteTick
        return QuoteTick(
            instrument_id=getattr(tick, "instrument_id"),
            bid_price=_coerce_price_obj(getattr(tick, "bid_price"), int(spec.price_precision)),
            ask_price=_coerce_price_obj(getattr(tick, "ask_price"), int(spec.price_precision)),
            bid_size=_coerce_qty_obj(getattr(tick, "bid_size"), int(spec.size_precision)),
            ask_size=_coerce_qty_obj(getattr(tick, "ask_size"), int(spec.size_precision)),
            ts_event=int(getattr(tick, "ts_event")),
            ts_init=int(getattr(tick, "ts_init")),
        )

    def _coerce_trade_precision(tick: Any, spec: Any) -> Any:
        N = load_nautilus_imports()
        TradeTick = N.TradeTick
        return TradeTick(
            instrument_id=getattr(tick, "instrument_id"),
            price=_coerce_price_obj(getattr(tick, "price"), int(spec.price_precision)),
            size=_coerce_qty_obj(getattr(tick, "size"), int(spec.size_precision)),
    ...
    def _write_batch(file_idx: int, msg: _Msg) -> None:
        ...
        if spec is not None and events:
            # CORE MISSION (2)(2): enforce numeric precision to match instrument contract.
            # This normalizes Binance fixed-width formatting like:
            #   "62100.12000000" -> "62100.12"
            #   "0.00100000"     -> "0.001"
            BarCls = N.Bar
            QuoteCls = N.QuoteTick
            TradeCls = N.TradeTick

            first = events[0]
            if isinstance(first, BarCls):
                events = [_coerce_bar_precision(b, spec) for b in events]
            elif isinstance(first, QuoteCls):
                events = [_coerce_quote_precision(t, spec) for t in events]
            elif isinstance(first, TradeCls):
                events = [_coerce_trade_precision(t, spec) for t in events]
...
# Optional (recommended) helper for manual construction:
# Keeps ergonomics (accepts InstrumentId or str) without violating msgspec union rules.
def as_instrument_id(x: object) -> InstrumentId:
    if isinstance(x, InstrumentId):
        return x
    if isinstance(x, str):
        s = x.strip()
        if not s:
    ...
    raise TypeError(f"instrument_id must be InstrumentId or str, got {type(x).__name__}")


class PhoenixLpiStrategyConfig(StrategyConfig, frozen=True):
    # IMPORTANT: msgspec does NOT support `InstrumentId | str`.
    # Unions containing a custom type may only include `None` (i.e. Optional[InstrumentId]).
    # Keep this as InstrumentId and rely on Nautilus' msgspec_decoding_hook which converts
    # JSON strings into InstrumentId via InstrumentId.from_str.
    instrument_id: InstrumentId
    trade_size: Decimal
...
class PhoenixLpiStrategy(Strategy):
    ...
    def on_start(self) -> None:
        ...
        # Optional but robust: if your BacktestDataConfig uses client_id for custom catalog data,
        # subscribing with the same client_id avoids versions/configurations where client-scoped
        # subscriptions are required for delivery.
        CUSTOM_DATA_CLIENT_ID = "CATALOG_CUSTOM"

        def _sub_custom(data_cls: Any) -> None:
            try:
                self.subscribe_data(
                    DataType(data_cls),
                    instrument_id=self.config.instrument_id,
                    client_id=CUSTOM_DATA_CLIENT_ID,
                )
            except TypeError:
                # Older Nautilus versions may not accept client_id here.
                self.subscribe_data(
                    DataType(data_cls),
                    instrument_id=self.config.instrument_id,
                )

        _sub_custom(BinanceOiMetrics)
        _sub_custom(BinanceFundingRate)
        _sub_custom(BinanceBookDepthPct)
...
class AlphaZScoreStrategyConfig(StrategyConfig, frozen=True):
    ...
    # Same msgspec limitation as PhoenixLpiStrategyConfig: do NOT use `InstrumentId | str`.
    # Strings in JSON are decoded into InstrumentId by Nautilus' msgspec_decoding_hook.
    instrument_id: InstrumentId
...
def build_backtest_data_configs(N: NautilusPhase4Imports, plan: DataLoadPlan) -> list[Any]:
    ...
    # Nautilus BacktestNode requires a client_id for non-"nautilus classes" (custom data types),
    # otherwise it raises:
    #   ValueError: Data type <...> not setup for loading into `BacktestEngine`
    CUSTOM_DATA_CLIENT_ID = "CATALOG_CUSTOM"
...
def _run_one(
    ...
    *,
    ...
    try:
        ...
        # If the backtest crashed mid-run, the engine may still be RUNNING.
        # BacktestNode.dispose() does not necessarily call engine.end() first, so disposing
        # can raise InvalidStateTrigger(RUNNING -> DISPOSE). Do a best-effort end.
        if not run_ok:
            try:
                eng = None
                try:
                    eng = node.get_engine(run_id) if run_id is not None else node.get_engine()
                except Exception:
                    eng = None

                if eng is not None:
                    trader = getattr(eng, "trader", None)
                    is_running_attr = getattr(trader, "is_running", None) if trader is not None else None
